...
def parse_pcap(pcap, pcap_path, file_name):
    ...
    with open(csv_file_path, 'w') as csv_file:
        ...
        for key, value in pcap_dict.items():
            writer.writerow(["".join([i+'.' for i in file_name.split(".")[:-1]])[:-1]] + list(key) + [value[0], len(value[1])] + value[1] + [None] + value[2])

    for k in pcap_dict:
        if len(pcap_dict[k][1]) > 2000:
            print(k, pcap_dict[k][0], len(pcap_dict[k][1]))
...
if __name__ == '__main__':
    ...
    file_list = [(i,i.split('\\')[-1]) for i in glob('D:\Infomedia_data_backups\Raw pcaps\\filtered_raw_dataset_temu2016_first_10_min\**\*.pcap')] #get_pcaps_list('D:\Infomedia_data_backups\Raw pcaps\\filtered_raw_dataset_temu2016_first_10_min', FILTER_LIST)
