...
from preprocess.MapGraphBuild import build
...
def main():
    ...
    for folder in folders:
        ...
        for i in d1:
            ...
            for group in gp.groups:
                d[folder][i+f"_{int(group)}"] = list(gp.get_group(group)['fname'])
    #
    ...
    for folder in d:
        ...
        #     continue
        c = 0
        folder_features = pd.DataFrame()
        folder_edges = pd.DataFrame()
        folder_graphs_labels = []
        for pre in d[folder]:
            print(c/len(d[folder]))
            c+=1
            print("started",pre)
            packets = []
            label = int(pre.split('_')[-1])
            folder_graphs_labels.append([c,label])
            srcs = set([h.replace("-",'.') for h in [[j for j in i.split("_") if j.count("-") == 3][0] for i in d[folder][pre]]])
            for file in d[folder][pre]:
                print("read",file)
                try:
                    ps = rdpcap(file)
                except:
            ...
            temp = pd.DataFrame(temp,columns=['source','target','weight','id'])
            folder_edges = folder_edges.append(temp,ignore_index=True)
            # D = getdegree(A)
            X = []
            for n in nodes:
            ...
            folder_features = folder_features.append(X,ignore_index=True)
            ...
            del packets,edges,nodes,X,t
        folder_graphs_labels = pd.DataFrame(folder_graphs_labels,columns=['id','label'])
        folder_graphs_labels.to_csv(folder+"/labels.csv")
        folder_features.to_csv(folder+"/features.csv")
        folder_edges.to_csv(folder+'/weights.csv')
    #
