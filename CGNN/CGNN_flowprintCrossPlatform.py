...
class cgnn_model(tfk.Model):
    def __init__(self, n_classes=140):
...
def main():
    folders = glob.glob("./data_crossPlatform/*/*")
    folders = list(filter(lambda x: ".pcap" not in x, folders))
    counterLabel = 1
    ...
    files_names = glob.glob("./data_crossPlatform/*/*/*.npy")
    ...
    result = tf.math.confusion_matrix(labels=_idx, predictions=idx,
                                      num_classes=140)  # use one hot to convert to 1D to do confMatrix
    print(result)
